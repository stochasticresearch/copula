...
resultsXY_bias = zeros( [size(subDependenciesVec) 7 ]);      % 7 entries, 1-5 are the different correction factors, 6 is matlab's built in tau, 7 is tau-b
resultsXY_var = zeros( [size(subDependenciesVec) 7 ]);
resultsYX_bias = zeros( [size(subDependenciesVec) 7 ]);
resultsYX_var = zeros( [size(subDependenciesVec) 7 ]);
...
for dependencyIdx=1:length(dependenciesVec)
    ...
    for subDependencyIdx=1:length(subDependencies)
        ...
        for xyOrientation=xyOrientationVec
            ...
            if(xyOrientation)
                resultsXY_bias(dependencyIdx, subDependencyIdx, 1:7) = tau_hat_bias;
                resultsXY_var(dependencyIdx, subDependencyIdx, 1:7) = tau_hat_bias;
            else
                resultsYX_bias(dependencyIdx, subDependencyIdx, 1:7) = tau_hat_bias;
                resultsYX_var(dependencyIdx, subDependencyIdx, 1:7) = tau_hat_bias;
            end
